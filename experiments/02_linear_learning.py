"""
Experiment 02: Linear Learning Limitation

Goal:
    Observe what happens when a single linear neuron is given
    training data that cannot be represented by a single weight.
...
The data changes from linear growth to a constant value
after input 5.

A single linear neuron cannot represent this relationship,
so we expect the loss to decrease but not reach zero.

Key learning:

    A model can only learn relationships that its current
    architecture is capable of representing.
"""
